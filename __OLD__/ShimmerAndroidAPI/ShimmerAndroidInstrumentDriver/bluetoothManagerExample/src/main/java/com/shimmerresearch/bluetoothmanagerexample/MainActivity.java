...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            phv = new PPGtoHRwithHRV(128);
            lpf = new Filter(Filter.LOW_PASS, 128, new double[] {5});
            hpf = new Filter(Filter.HIGH_PASS, 128, new double[] {0.5});
        } catch (Exception e) {
            ...
            switch (msg.what) {
                case ShimmerBluetooth.MSG_IDENTIFIER_DATA_PACKET:
                    if ((msg.obj instanceof ObjectCluster)) {
                        ...
                        double ts = timeStampCluster.mData;
                        Log.i(LOG_TAG, "Time Stamp: " + ts);
                        ...
                        if (a13Cluster!=null) {
                            a13Data = a13Cluster.mData;
                            Log.i(LOG_TAG, "A13: " + a13Data);
                        }

                        //Process PPG signal and calculate heart rate
                        try {
                            double dataFilteredLP = lpf.filterData(a13Data);
                            double dataFilteredHP = hpf.filterData(dataFilteredLP);
                            int heartRate = (int)Math.round(phv.ppgToHrConversion(dataFilteredHP, ts));
                            Log.i(LOG_TAG, "HR: " + heartRate);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    ...
                    switch (state) {
                        case CONNECTED:
                            ...
                            if(shimmerDevice != null) {
                                ...
                                shimmerDevice.setSamplingRateShimmer(128);
                            }
